...
export class ListPropertiesComponent implements OnInit, OnChanges {
  ...
  if (this.filters) {
    localStorage.setItem('filtrosBuscaImoveis', JSON.stringify(this.filters));
  }
}

private carregarFiltrosSalvos(): void {
  // Carrega filtros salvos, se existirem
  const filtrosSalvos = localStorage.getItem('filtrosBuscaImoveis');
  if (filtrosSalvos) {
    this.filters = JSON.parse(filtrosSalvos);
    this.generateForm();
    //this.carregarFiltrosSalvos(); // novo 
    this.getProperties();
  }
}
  ...
  private getProperties(): void {
    if (this.filters.neighborhood && !Array.isArray(this.filters.neighborhood)) {
  this.filters.neighborhood = [Number(this.filters.neighborhood)];
}
  ...
  public searchProperties(): void {
    ...
    // Atualiza a URL com os filtros aplicados para permitir navegação com histórico
    this.router.navigate([], {
      relativeTo: this.route,
      queryParams: this.filters,
      queryParamsHandling: 'merge'
    });
    this.salvarFiltros(); // NOVO LOCALSTORAGE
    this.getProperties();
  }
